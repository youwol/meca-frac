...
const RoseSection = (props: {
  ...
  const handleDeleteRoseElement = (id: string) => {
    ...
    const filterGroup = (prevGroup: Section[], validSections: Section[]) => {
...
const RoseSections = ({
  ...
  useEffect(() => {
    setObsColor((prevColor) => (prevColor !== obsColor ? obsColor : prevColor));
    setSimColor((prevColor) => (prevColor !== simColor ? simColor : prevColor));
  }, [obsColor, simColor]);

  return (
    <div className={"rose-sections"}>
      <div className={"d-flex "}>
        <div className={"col-6"} style={{ borderRight: "5px solid white" }}>
          <CircleColorSection
            title={"Observation"}
      ...
      <div className={"d-flex flex-column"}>
        {sections.map((section: Section, index: number) => (
          <RoseSectionElements
            ...
            color={color}
            label={section.label}
            onDeleteSection={index > 0 ? onDeleteRoseElem : undefined}
          />
        ))}
      </div>
    </div>
...
const RoseSectionElements = ({
  ...
  useEffect(() => {
    if (parentRef.current) {
      appendRose(parentRef.current, roseId1.current, false, color.observation);
      appendRose(parentRef.current, roseId2.current, true, color.simulation);
    }
  }, [color]);
  ...
  return (
    <>
      ...
      >
        ...
        {onDeleteSection ? (
          <div
            className={"d-flex col-6 justify-content-end border-left-2"}
